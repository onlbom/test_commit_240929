...
myCharacter.ep += 50;
document.write(`<p>현재 나의 경험치 : ${myCharacter.ep}</p>`);

myCharacter.hp += 100;
document.write(`<p>현재 나의 제력 : ${myCharacter.hp}</p>`);

// 날짜 객체
// Time Server(수많은 인공위성 중에서 근접한 4개로부터 좌표값을 받아옴)로 부터 동기화시킨 값을 브라우저 표현 및 활용
...
// D-day 계산시, 24시간동안 팝업 열지 않기, 사용자가 동작수행을 멈추었을 때부터 4시간 경과시 자동 로그인
...
var num_01 = 300000;
var ch_standardNum = num_01.toExponential();
...
var $num = 1234.5678;
var $round = Math.round($num); // 반올림
document.write(`<p>소수점 첫번째 자리에서 반올림 한 값 : ${$round}</p>`);

var $ceil = Math.ceil($num); // 올림
document.write(`<p>소수점 첫번째 자리에서 올림 한 값 : ${$ceil}</p>`);

var $floor = Math.floor($num); // 내림
document.write(`<p>소수점 첫번째 자리에서 내림 한 값 : ${$floor}</p>`);

var $random = Math.random();
document.write(`<p>0초과 1미만의 실수를 무작위로 반환한 값 : ${$random}</p>`);

// 1~10 사이의 값을 랜덤으로 받환시켜야 함
...
// 문자 객체(string) : 특정 문자 데이터를 가져와서 요구사항에 맞춰 조작
...
console.log(numBuyPrice_01); // NaN (숫자가 아닌 경우 뜸)

var numBuyPrice_02 = Number(buyPrice.replace(",", "")); // ","를 찾아서 바꿀 위치에 아무것도("") 넣지마~!
